...
const char* WIFI_SSID = "Tecno pop";
const char* WIFI_PASSWORD = "tecnopop";
// =========================
// MQTT Settings (sync with src/network_config.py)
// =========================
const char* MQTT_SERVER = "157.173.101.159";
const uint16_t MQTT_PORT = 1883;

const char* MQTT_TOPIC = "vision/team213/movement";
const char* MQTT_CLIENT_ID_PREFIX = "esp32_team213";

// =========================
// Servo Configuration
// =========================
const uint8_t SERVO_PIN = 5; // Recommended ESP32 pin
...
const float TRACK_STEP = 0.22f;
const float SCAN_STEP = 0.40f;

const unsigned long TRACK_INTERVAL_MS = 32;
const unsigned long SCAN_INTERVAL_MS = 28;
const unsigned long COMMAND_TIMEOUT_MS = 1500;

// Set true if MOVE_LEFT / MOVE_RIGHT are still backwards on your mount
...
MovementCommand parseCommand(String message) {
  ...
  // JSON from vision_node.py: {"status":"MOVE_LEFT",...}
  if (upper.indexOf("MOVE_LEFT") >= 0) {
    return CMD_LEFT;
  }
  if (upper.indexOf("MOVE_RIGHT") >= 0) {
    return CMD_RIGHT;
  }
  if (upper.indexOf("CENTERED") >= 0) {
    return CMD_CENTER;
  }
  if (upper.indexOf("NO_FACE") >= 0) {
    return CMD_SCAN;
  }

  // Plain serial / test commands
